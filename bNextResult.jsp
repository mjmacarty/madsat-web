...
<%@page import="java.util.*"%>
<%@page import="java.util.*"%>
<%@page import="madsat2.controller.CustomSearch"%>
<%@page import="madsat2.controller.HTMLDocParser"%>
<%@page import="madsat2.controller.CustomSearchResults.Items"%>
...
<table class="resultTable">
	<thead>
		...
		%>
		<tr>
			<th colspan="0">NO RESULTS FOUND FROM EXISTING DATABASES</th>
		</tr>
	</thead>
	<%
	...
	<th colspan="0">Google Search Results for Query: <br><%=origQuery%>
	</th>


	<%
		int i = 0;
		List<Items> googleSearchRes = null;
		try {
			googleSearchRes = CustomSearch.read(CustomSearch
					.makeSearchString(origQuery, 1, 8));
			for (Items urlStr : googleSearchRes) {
	%>
	<tbody>
	<tr>
		<td style="width: 130px"><a href="#"
			onClick="googleSearchWindow('<%=urlStr.getLink()%>')"><%=urlStr.getLink()%>
		</a></td>
		<%
			if (urlStr.getLink().endsWith("pdf")) {
		%>
		<td>pdf not summarized</td>
		<td></td>
		</td>
	</tr>
	<%
		} else {
					//  String title= new HTMLDocParser().parseHTML(urlStr.getLink(),dirPath2,i );
					String title = new HTMLDocParser().parseHTML(
							urlStr.getLink(), sessionDir, i);
					String summaryURL = sessionDir + "/summary" + i
							+ ".html";
					String aTextSummaryURL = sessionDir + "/aTextSummary"
							+ i + ".html";
					i++;
	%>

	<td><a href="#" onClick="googleSearchWindow('<%=summaryURL%>')"><%=title%>
	</a></td>
	<td colspan="0"><%=urlStr.getSnippet()%></td>
	</td>
	</tr>
	</tbody>
	<%
	...
	<tbody>
	<tr>
		<td>Cannot get results from <%=e.getMessage()%>
		</td>
	</tr>
	<%
		}
